...
wait_for() {
  ...
  until wget -T "${PROBE_TIMEOUT}" -qO- "${url}" >/dev/null 2>&1; do
...
check_json() {
  ...
  body=$(wget -T "${PROBE_TIMEOUT}" -qO- "${url}" 2>/dev/null) || { echo "ERROR: could not reach ${url}"; exit 1; }
...
ui=$(wget -T "${PROBE_TIMEOUT}" -qO- "${PIM_URL}/") || { echo "ERROR: could not reach ${PIM_URL}/"; exit 1; }
