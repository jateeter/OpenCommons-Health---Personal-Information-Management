...
export class EpicIntegrationService {
  ...
  async connectStart(): Promise<Record<string, unknown>> {
    ...
    const record = this.withAudit({
      ...
      audit: [],
    }, 'connect-start', 'ok', 'Started Epic SMART authorization flow.');
    ...
    const authorizationUrl = this.config.mode === 'mock'
      ...
      : `${fhirBaseUrl}/oauth2/authorize?response_type=code&client_id=${encodeURIComponent(this.config.clientId ?? '')}&redirect_uri=${encodeURIComponent(callback)}&scope=${encodeURIComponent(this.config.scopes.join(' '))}&state=${encodeURIComponent(state)}&aud=${encodeURIComponent(fhirBaseUrl)}`;
  ...
  async connectCallback(query: URLSearchParams): Promise<EpicConnectionPublicStatus> {
    ...
    }, 'connect-callback', 'ok', 'Epic SMART authorization completed and encrypted grant stored in the owner pod.');
    await this.connectionRepository?.save(record);
    return this.publicStatus(record);
  }
  ...
  async preview(_body: Record<string, unknown> = {}): Promise<EpicImportPreview> {
    ...
    return {
      ...
      changes: mapEpicResourcesToPim(mockAnnualWellnessResources(), {
